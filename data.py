...
class RandomPatch(torch.nn.Module):
    ...
    def __call__(self, image: torch.Tensor):
        ...
        if (not self.force_randomcolor) and self.prob_median > random.random():
            image_patch[:, :, :] = self._expand_as_tensor(image.view(
                image.shape[0], -1).median(dim=1).values, image_patch)
        else:
            random_color = [torch.randint(low, high, size=(1,)) for low, high in zip(
                self.color_interval[0], self.color_interval[1])]
            random_color = torch.Tensor(random_color)
            image_patch[:, :, :] = self._expand_as_tensor(
                random_color, image_patch)
        return image
...
if __name__ == "__main__":
    ...
    image = Image.open(
        "ImageSet/train/72/11_Memmi_Madonna_1000dpi__0014_Livello 15.tif")
    image = np.array(image)
    image_tensor = PILToTensor()(image)
    image_patch = RandomPatch(
        10, 40, [[0, 0, 0], [100, 100, 100]], .5, force_randomcolor=True)(image_tensor)
    image_orig = Image.fromarray(
        image_patch.permute(1, 2, 0).numpy().astype("uint8"))
