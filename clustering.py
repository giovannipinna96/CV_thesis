...
def kmenas_cluster(X, n_clusters=16, save=True):
    ...
    if save:
        utils.save(y_predicted, 'kmenas_cluster_y_predicted')
        utils.save(centers_cluster, 'kmenas_cluster_centers_cluster')
        utils.save(all_distances, 'kmenas_cluster_all_distances')
    return y_predicted, centers_cluster, all_distances


def fuzzy_cluster(X_train, X_test, n_clusters=16, save=True):
    ...
    if save:
        utils.save(y_hard, 'fuzzy_cluster_y_hard')
        utils.save(y_soft, 'fuzzy_cluster_y_soft')
    return y_hard, y_soft


def agglomerative_cluster(X, n_clusters=16, affinity='euclidean', linkage='complete', save=True):
    X_std = scale_features(X)
    ac = AgglomerativeClustering(
        n_clusters=n_clusters, affinity=affinity, linkage=linkage)
    labels = ac.fit_predict(X_std)
    if save:
        utils.save(labels, 'agglomerative_cluster_labels')
    return labels


def dbscan_cluster(X, eps=0.2, min_samples=5, metric='euclidean', save=True):
    X_std = scale_features(X)
    db = DBSCAN(eps=eps, min_samples=min_samples, metric=metric)
    y_db = db.fit_predict(X_std)
    if save:
        utils.save(y_db, 'dbscan_cluster')
    return y_db
