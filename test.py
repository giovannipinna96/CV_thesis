...
# def test_model_triplet(model, dataloader, performance=train.accuracy, loss_triplet_fn=None, device=None, loss_type = None):
#     # for tensorboard
#     writer2 = SummaryWriter(f'runs/punzoni/test_model_tensorboard')
#     step = 0
#     # create an AverageMeter for the loss if passed
#     if loss_triplet_fn is not None:
#         loss_meter = AverageMeter()
...
#             y_hat = model(X)
#             loss = loss_triplet_fn(y_hat[:X_anchor_dim], y_hat[X_anchor_dim:X_anchor_dim*2], y_hat[X_anchor_dim*2:]) if loss_triplet_fn is not None else None
#             acc = performance(y_hat, y)
#             if loss_triplet_fn is not None:
#                 loss_meter.update(loss.item(), X.shape[0])
#             performance_meter.update(acc, X.shape[0])
...
#     # get final performances
#     fin_loss = loss_meter.sum if loss_triplet_fn is not None else None
#     fin_perf = performance_meter.avg
#     print(f"TESTING - loss {fin_loss if fin_loss is not None else '--'} - performance {fin_perf:.4f}")
