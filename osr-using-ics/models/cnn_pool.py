...
from keras.applications import vgg16, vgg19, xception, densenet, inception_v3
...
def build_cnn(img_height=None, num_channel=None, reg=None, latent_fea=None, num_normal_class=None, cnn_type=None):
    ...
    acti_alpha = 0.2
    set_bias = False
    weights_init = tf.keras.initializers.TruncatedNormal(mean=0, stddev=0.01)
    ...
    if cnn_type == 'alexnet':
        ...
        latent_layer = ResNet50(include_top=False, weights=None,
...
def train_logits_cnn(data=None, label=None, normal_class=None, reg=None, epoch=None, batch_size=None, name=None, cnn_type=None):
    ...
    model_set = build_cnn(img_height=img_height, num_channel=num_channel, reg=reg,
                          latent_fea=256, num_normal_class=num_normal_class, cnn_type=cnn_type)

    cnn = model_set[0]
    cnn_latent = model_set[1]

    customized_optimizer = tf.keras.optimizers.RMSprop(learning_rate=1e-4, decay=1e-9)
