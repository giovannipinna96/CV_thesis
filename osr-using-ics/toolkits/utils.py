...
def get_data(dataset=None, normal_class=None, data_format=None, preprocess='minmax'):
    ...
    y_train = np.reshape(y_train[:100], newshape=(-1,))
    y_test = np.reshape(y_test[:100], newshape=(-1,))

    x_train = _reshape_data(data=x_train[:100], data_shape=data_format, num_channels=num_channel)
    x_test = _reshape_data(data=x_test[:100], data_shape=data_format, num_channels=num_channel)
...
def get_my_data(normal_class=None):
    ...
    if normal_class is None:
        ...
        data = {'x_train_normal': train_set[0][:300], 'x_train_abnormal': train_set[1][:300],
                'y_train_normal': train_set[2][:300], 'y_train_abnormal': train_set[3][:300],
                'x_test_normal': test_set[0][300:400], 'x_test_abnormal': test_set[1][300:400],
                'y_test_normal': test_set[2][300:400], 'y_test_abnormal': test_set[3][300:400]}
...
def split_data(model_name=None, data=None, rho=None, split_method=None, ground_truth=None, normal_class=None):
    ...
    if split_method == 'cnn':
        ...
        sim_score = np.pad(gt, ((0, 0), (0, abs(pred.shape[1] - gt.shape[1]))), mode='constant',
                           constant_values=0) * pred * probability
