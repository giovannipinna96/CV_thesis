    ...
    with torch.no_grad():
        for X, y in tqdm(dataloader):
            ...
            for j in range(outlier_score_val.shape[0]):
                ...
                if (outlier_score_val[j] <= threshold2):# and (outlier_score_val[j] <= threshold2) :
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weights_save_path", type=str, default="models/model_BEST5.pt")
...
#    print(outlier_scores_extra)
    ...
    print('Saving pickle')
    utils.save_obj(file_name=f"./pickle_thres_mean_BEST5",
                        first=threshold,
                        second=mean
                        )
